...
Run the plugin's doctor (`node lib/scripts/ct_doctor.js check`) before
guessing. It reports PASS / WARN / FAIL per check, each with a fix hint:
...
7. **Harness registration** — the `mcp["curated-thoughts"]` entry and the
   loader file `~/.config/opencode/plugins/curated-thoughts.js` pointing at a
   readable payload, plus the three skill directories under
   `~/.config/opencode/skills/`, so the tools and skills actually load.
...
The sidecar is registered as `mcp["curated-thoughts"]` in the global
`~/.config/opencode/opencode.json[c]` with `--mcp` arguments, and the plugin
loads from the loader file
`~/.config/opencode/plugins/curated-thoughts.js`, which points at the payload
directory under `${XDG_DATA_HOME:-~/.local/share}/curated-thoughts/opencode/`.
All three pieces matter: without the `mcp` entry there are no tools, without
the loader file the health snapshot never loads, and without the skill
directories under `~/.config/opencode/skills/` the skills never appear.
`scripts/install.sh` prints the proposed merge by default and only writes it
under `CT_INSTALL_EDIT=1` — it merges the `mcp` entry only if absent, never
overwrites an existing entry, and never touches the `plugin` config array.
If both a plugin-installed copy and a loose
`~/.config/opencode/skills/curated-thoughts*` skill exist, remove the loose
copy to avoid skill collisions (the installer warns; it never deletes user
files). Verify with `node lib/scripts/ct_doctor.js check`: check 7 reports
`UNKNOWN` when an entry is simply absent from the global config (OpenCode also
merges project and managed config the doctor cannot see) and FAILs only on
local, unambiguous problems such as a loader pointing at a missing payload.
