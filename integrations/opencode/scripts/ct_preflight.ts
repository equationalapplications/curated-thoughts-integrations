 ...
 *
 * `better-sqlite3` is a devDependency and loaded LAZILY (Step 5.5): the
 * shipped tarball carries no node_modules, and a missing native build must
 * degrade only the census (doctor check 8), never crash the doctor at
 * import time. Every entry point that touches the DB funnels through
 * `_requireDatabase()`, which resolves to a structured error on failure.
 */
...
function _openReadonly(
  ...
  /** Open the brain read-only (Step 5.7). Never throws: a missing or
   * unloadable better-sqlite3 becomes a structured error so only the
   * census degrades — the doctor must keep producing JSON output. */
...
export function hasEvidenceTable(dbPath: string): boolean | null {
  ...
  */
  ...
  try {
    const opened = _openReadonly(dbPath);
    if (opened.conn === null) return null;
    conn = opened.conn;
  } catch {
    return null;
  }
