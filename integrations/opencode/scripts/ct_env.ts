 ...
 * the local-readiness probe, and the OpenCode plugin entry point.
 *
 ...
 *    only in `/usr/bin`.
 *
 * ct_env never spawns the sidecar — that costs an MCP handshake — and never
 * opens the brain database.
 *
...
export function findSidecar(
   ...
   * without re-deriving the search order. Never spawns the sidecar — that
   * costs an MCP handshake — and never opens the brain database.
   */
