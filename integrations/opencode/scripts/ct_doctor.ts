/**
 * ct_doctor.ts — install/health doctor for the Curated Thoughts OpenCode
 * plugin.
 *
 ...
 *   - opencode-registration reads the GLOBAL `opencode.json[c]` with a
 *     JSONC-tolerant parse for the `mcp["curated-thoughts"]`
 *     entry, plus two independent pieces of local evidence: the loader file
 *     `~/.config/opencode/plugins/curated-thoughts.js` AND the payload it
 *     names, and the three skill directories under
 *     `~/.config/opencode/skills/`. The doctor reads the global config ONLY:
 *     OpenCode merges remote, project, `.opencode/`, OPENCODE_CONFIG,
 *     OPENCODE_CONFIG_CONTENT and managed sources it cannot see, so a
 *     missing entry is UNKNOWN with the scope stated — never a confident
 *     FAIL — and the doctor never boots OpenCode to find out (spec §6,
 *     §11).
 *
 ...
 *
 * Spec: docs/superpowers/specs/2026-09-17-opencode-integration-design.md §6.
...
export type Status = typeof PASS | typeof WARN | typeof FAIL | typeof UNKNOWN;
...
// OpenCode registration paths (registration.ts is the binding source; the
// doctor re-derives them from the environment the same way so it can run
// standalone from lib/scripts without importing the installer).
const MCP_NAME = 'curated-thoughts';
const LOADER_FILENAME = 'curated-thoughts.js';
const SKILL_NAMES: readonly string[] = [
  'curated-thoughts-usage',
  'curated-thoughts-ops',
  'curated-thoughts-sidecar',
];
// The mcp entry shape the installer writes (registration.ts's McpEntry).
const EXPECTED_MCP_TYPE = 'local';
const EXPECTED_MCP_COMMAND: readonly string[] = ['curated-thoughts-mcp', '--mcp'];
...
/** Parse the global config. Returns (root|null, error|null). JSONC-tolerant:
 * OpenCode's config is JSONC (comments and trailing commas are legal), and a
 * strict JSON.parse would false-FAIL a perfectly healthy config.
 *
 * The tolerance is a small state-machine sanitizer rather than the
 * jsonc-parser dependency: `lib/scripts/ct_doctor.js` must run from the
 * release tarball with no node_modules (spec §2 — the doctor's only
 * permitted third-party module is better-sqlite3, lazily required by the
 * pre-flight). Strings, escapes, line/block comments and trailing commas
 * are handled explicitly; anything else JSON.parse rejects for us.
 */
function _parseJsonc(text: string): unknown {
  // Pass 1: strip comments outside strings, compacting to nothing so
  // JSON.parse never sees them. Positions outside comments are preserved
  // byte-for-byte.
  let out = '';
  let i = 0;
  const n = text.length;
  ...
  // Pass 2: drop trailing commas (a comma followed only by whitespace and
  // the matching closer). Nested shapes are handled in one pass because the
  // regex only fires when the very next token is a closer.
  out = out.replace(/,(\s*[}\]])/g, '$1');
  return JSON.parse(out);
}

/** Parse the global config from text. */
function _parseGlobalConfigText(
  text: string,
): { root: Record<string, unknown> | null; error: string | null } {
  let root: unknown;
  try {
    root = _parseJsonc(text);
  } catch (e) {
    return { root: null, error: `could not parse (${(e as Error).message.slice(0, 80)})` };
  }
  if (root === null || typeof root !== 'object' || Array.isArray(root)) {
    return { root: null, error: 'top level is not a JSON object' };
  }
  return { root: root as Record<string, unknown>, error: null };
}
 ...
 *   b. the loader file present AND the payload it names readable — local,
 *      unambiguous evidence, so a loader pointing at a missing payload is
 *      FAIL with a reinstall hint, exempt from the scope caveat below;
 *   c. the three skill directories under the global skills path.
 *
 * Scope statement, printed with the result: the doctor reads the GLOBAL
 * config only. OpenCode also merges remote, project, `.opencode/`,
 * OPENCODE_CONFIG, OPENCODE_CONFIG_CONTENT and managed sources it cannot
 * see, so a missing mcp entry is UNKNOWN — never FAIL — and the doctor
 * never boots OpenCode to find out (spec §11).
 */
export function checkOpencodeRegistration(
  env: NodeJS.ProcessEnv = process.env,
): CheckResult {
  const home = env.HOME || env.USERPROFILE || homedir();
  const configDir = join(_xdgDir(env.XDG_CONFIG_HOME, join(home, '.config')), 'opencode');
  ...
  // --- (a) mcp entry in the global config (UNKNOWN-capable) ---------------
  let mcpVerdict = 'present';
  let mcpDetail: string | null = null;
  const st = (() => { try { return lstatSync(configPath); } catch { return null; } })();
  if (st === null) {
    mcpVerdict = 'not found';
  } else if (!st.isFile()) {
    mcpVerdict = 'unreadable';
    mcpDetail = `${configPath} exists but is not a regular file`;
  } else {
    ...
    if (text === null) {
      mcpVerdict = 'unreadable';
      mcpDetail = `${configPath} could not be read`;
    } else {
      ...
      if (root === null) {
        ...
        if (mcp !== undefined && !_isPlainObject(mcp)) {
          ...
          if (entry === undefined) {
            mcpVerdict = 'not found';
          } else if (!_isPlainObject(entry)) {
            mcpVerdict = 'unreadable: the mcp entry is not an object';
          } else {
  ...
  // --- (b) loader file + payload (local evidence, FAIL-capable) -----------
  const loaderPath = join(pluginsDir, LOADER_FILENAME);
  let loaderVerdict = 'missing';
  let loaderDetail = '';
  const lst = (() => { try { return lstatSync(loaderPath); } catch { return null; } })();
  if (lst !== null && !lst.isFile()) {
    loaderVerdict = 'unreadable';
    loaderDetail = `${loaderPath} exists but is not a regular file`;
  } else if (lst !== null) {
    let contents: string | null = null;
    try {
      contents = readFileSync(loaderPath, { encoding: 'utf8' });
    } catch {
      contents = null;
    }
    if (contents === null) {
      ...
      if (target === null) {
        ...
        // The loader re-exports from the payload ENTRY file
        // (<payloadDir>/lib/src/index.js) directly — the file: URL names the
        // entry, so payload presence is the entry's presence.
        if (!existsSync(target)) {
          loaderVerdict = 'payload-missing';
          loaderDetail = `${loaderPath} points at ${target}, which is missing`;
          hints.push(
            `Re-install: run scripts/install.sh from the release tarball to restore the payload it belongs to, or remove ${loaderPath} if OpenCode is no longer used with Curated Thoughts.`,
          );
        } else {
          loaderVerdict = 'present';
        }
      }
    }
  }
  ...
  const skillsPresent = missingSkills.length === 0;

  // --- combine -------------------------------------------------------------
  const scope =
    'The doctor reads the global config only; OpenCode also merges project, '
    + '`.opencode/`, OPENCODE_CONFIG, OPENCODE_CONFIG_CONTENT, remote and '
    + 'managed config it cannot see.';

  // Loader problems are local, unambiguous evidence and FAIL regardless of
  // the mcp verdict — a loader pointing at a missing payload specifically
  // gets the reinstall hint (spec §6 check 7b). Missing skills stay below:
  // they never FAIL, they downgrade the verdict to WARN with the reinstall
  // hint (the spec reserves FAIL for the loader payload and unreadable
  // config cases).
  const localFails: string[] = [];
  if (loaderVerdict === 'payload-missing') {
    localFails.push(`loader file points at a missing payload: ${loaderDetail}`);
  } else if (loaderVerdict === 'unreadable') {
    localFails.push(`loader file unreadable: ${loaderDetail}`);
  }

  const reinstallHint =
    'Re-run the install to repair it: download the opencode-v<version> release '
    + 'tarball, unpack, run scripts/install.sh to preview, then '
  ...
  if (localFails.length > 0) {
    return _fail(
      'opencode-registration',
      localFails.join('; '),
      hints.length > 0 ? hints.join(' ') : reinstallHint,
    );
  }

  if (mcpVerdict === 'not found') {
    return {
      name: 'opencode-registration',
      status: UNKNOWN,
      detail: `mcp["${MCP_NAME}"] not found in ${configPath}; OpenCode also merges `
        + 'project and managed config the doctor cannot see'
        + (loaderVerdict === 'present' ? `; loader file present at ${loaderPath}` : '')
        + (skillsPresent ? `; all three skills present under ${skillsDir}` : ''),
      hint: `Re-run install.sh (preview) and apply with CT_INSTALL_EDIT=1 to add the entry, or merge it manually. ${scope}`,
    };
  }
  if (mcpVerdict === 'disabled') {
    return _warn(
      'opencode-registration',
  ...
  if (mcpVerdict !== 'present') {
    // 'unreadable' entry-object variants reachable through odd shapes.
    return _fail(
      'opencode-registration',
      `mcp["${MCP_NAME}"] in ${configPath}: ${mcpVerdict}`,
      `Fix the entry, or re-run the installer. ${scope}`,
    );
  }

  // Everything green — say what was actually verified, piece by piece.
  const loaderNote =
    loaderVerdict === 'present'
      ? `loader file ${loaderPath} present and its payload readable`
      : loaderVerdict === 'unrecognized'
        ? `${loaderDetail} (the mcp entry is what mounts the server)`
        : `loader file absent from ${pluginsDir} (the mcp entry is what mounts the server)`;
  const skillsNote = skillsPresent
    ? `all three skills present under ${skillsDir}`
    : `skills missing under ${skillsDir}: ${missingSkills.join(', ')}`;
  if (!skillsPresent) {
    return _warn(
      'opencode-registration',
      `mcp["${MCP_NAME}"] present in ${configPath} with the expected shape; ${loaderNote}; ${skillsNote}`,
      reinstallHint,
    );
  }
  if (loaderVerdict !== 'present') {
    return _warn(
      'opencode-registration',
      `mcp["${MCP_NAME}"] present in ${configPath} with the expected shape; ${loaderNote}; ${skillsNote}`,
      loaderVerdict === 'unrecognized' ? `${loaderDetail}. ${reinstallHint}` : reinstallHint,
    );
  }
  return _pass(
    'opencode-registration',
...
export function formatText(results: CheckResult[]): string {
  ...
  const nUnknown = results.filter((r) => r.status === UNKNOWN).length;
  const nPass = results.length - nFail - nWarn - nUnknown;
  lines.push(
    `\n${nFail} FAIL, ${nWarn} WARN, ${nUnknown} UNKNOWN, ${nPass} PASS — exit ${exitCodeFor(results)}`,
  );
  return lines.join('\n');
}
...
function _printHelp(): void {
  process.stdout.write(
    `ct_doctor.ts — Curated Thoughts OpenCode plugin doctor (read-only).\n`
      + '\nUsage:\n'
